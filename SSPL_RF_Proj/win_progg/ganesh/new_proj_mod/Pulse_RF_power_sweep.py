...
class SweepWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            if self.pm_instr:
                try:
                    self.log_msg.emit("Attempting to fetch peak power from power meter...")
                    time.sleep(0.3)
                    # Configure to capture max power
                    self.pm_instr.write("INIT:CONT OFF")
                    self.pm_instr.write("INIT")
                    self.pm_instr.write("*WAI")  # Wait for measurement
                    time.sleep(0.3)

                    # Try both options depending on model
                    try:
                        raw = self.pm_instr.query("FETCH?")
                        values = [float(v) for v in raw.strip().split(",") if v]
                        if values:
                            live_power = max(values)
                        else:
                            self.log_msg.emit("[WARNING] No valid values received from power meter.")
                            live_power = None

                    except:
                        try:
                            live_power = float(self.pm_instr.query("FETCH?"))
                        except Exception as e:
                            self.log_msg.emit(f"[ERROR] Failed to fetch power: {e}")
                            live_power = None

                except Exception as e:
                    self.log_msg.emit(f"[WARNING] Power meter peak read failed: {e}")
...
class NGP800IVSweepApp(QWidget):
    ...
    def setup_records_ui(self):
        ...
        self.records_table.setHorizontalHeaderLabels([
            ...
            "RF Freq (Hz)", "PowerIN (dBm)", "PowerOUT Max (dBm)",
    ...
    def setup_config_ui(self):
        ...
        # --- Pulse Configuration Section ---
        pulse_layout = QGridLayout()

        pulse_layout.addWidget(QLabel("Pulse Period:"), 0, 0)
        self.pulse_period_input = QLineEdit("1")
        self.pulse_period_unit = QComboBox()
        self.pulse_period_unit.addItems(["s", "ms", "us", "ns"])
        pulse_layout.addWidget(self.pulse_period_input, 0, 1)
        pulse_layout.addWidget(self.pulse_period_unit, 0, 2)

        pulse_layout.addWidget(QLabel("Pulse Width:"), 1, 0)
        self.pulse_width_input = QLineEdit("0.5")
        self.pulse_width_unit = QComboBox()
        self.pulse_width_unit.addItems(["s", "ms", "us", "ns"])
        pulse_layout.addWidget(self.pulse_width_input, 1, 1)
        pulse_layout.addWidget(self.pulse_width_unit, 1, 2)

        pulse_layout.addWidget(QLabel("Pulse Delay:"), 2, 0)
        self.pulse_delay_input = QLineEdit("0")
        self.pulse_delay_unit = QComboBox()
        self.pulse_delay_unit.addItems(["s", "ms", "us", "ns"])
        pulse_layout.addWidget(self.pulse_delay_input, 2, 1)
        pulse_layout.addWidget(self.pulse_delay_unit, 2, 2)

        # Pulse On Button
        self.pulse_on_button = QPushButton("Enable Pulse")
        self.pulse_on_button.clicked.connect(self.enable_pulse_mode)
        pulse_layout.addWidget(self.pulse_on_button, 3, 0, 1, 3)
    ...
    def emergency_stop_all(self):
        ...
        try:
            ...
            if self.nrx_instr_type != "NRX":  # Only older models like NRP2 might accept OUTP OFF
    ...
    def enable_pulse_mode(self):
        try:
            ...
            # Convert pulse values to seconds
            period = float(self.pulse_period_input.text()) * unit_map[self.pulse_period_unit.currentText()]
            width = float(self.pulse_width_input.text()) * unit_map[self.pulse_width_unit.currentText()]
            delay = float(self.pulse_delay_input.text()) * unit_map[self.pulse_delay_unit.currentText()]

            # Send SCPI commands to EXG
            self.exg_instr.write("PULM:STAT ON")
            self.exg_instr.write("PULM:SOUR INT")
            self.exg_instr.write(f"PULM:INT:PER {period}")
            self.exg_instr.write(f"PULM:INT:WIDT {width}")
            self.exg_instr.write(f"PULM:INT:DEL {delay}")
            self.exg_instr.write("PULM:INT:POL NORM")

            self.log("Pulse mode enabled with configured parameters.")
        except Exception as e:
            self.log(f"[ERROR] Failed to configure pulse: {e}")
            QMessageBox.warning(self, "Pulse Error", f"Pulse setup failed: {e}")
    ...
    def stop_sweep(self):
        ...
        if self.rf_control_screen:
            ...
            if self.exg_instr:
                try:
                    self.exg_instr.write("PULM:STAT OFF")
                    self.log("Pulse mode disabled on EXG.")
                except Exception as e:
                    self.log(f"Failed to disable pulse mode: {e}")
