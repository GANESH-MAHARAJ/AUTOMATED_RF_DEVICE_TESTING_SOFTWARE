...
from PyQt6.QtWidgets import (
    ...
    QVBoxLayout, QHBoxLayout, QComboBox, QGridLayout, QMessageBox, QStackedLayout, QTableWidgetItem,QFileDialog,QListView
)
import random
...
class MultiInstrumentConnectScreen(QWidget):
    ...
    def init_ui(self):
        ...
        layout.addWidget(self.ngp800_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        layout.addWidget(self.exg_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        layout.addWidget(self.nrx_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        # Title label styling
        layout.itemAt(0).widget().setStyleSheet(label_style)

        # Status labels
        for lbl in [self.ngp800_status, self.exg_status, self.nrx_status]:
            lbl.setStyleSheet(status_style)

        # Buttons
        for btn in [self.ngp800_button, self.exg_button, self.nrx_button]:
...
class NGP800IVSweepApp(QWidget):
    ...
    def __init__(self):
        ...
        self.thread_pool = []  # to hold thread references
    ...
    def setup_records_ui(self):
        ...
        header.setMinimumSectionSize(60)  # fallback if user resizes too small
        header.setStretchLastSection(False)

        # Set each column to Interactive mode + min width
        for i, width in enumerate(column_widths):
            header.setSectionResizeMode(i, QHeaderView.ResizeMode.Interactive)
            self.records_table.setColumnWidth(i, width)  # set default width (can grow/shrink)
    ...
    def setup_config_ui(self):
        ...
        rf_layout.addWidget(QLabel("Freq Start (MHz):"), 0, 0)
        self.freq_start_input = QLineEdit("0")
        rf_layout.addWidget(self.freq_start_input, 0, 1)

        rf_layout.addWidget(QLabel("Step (MHz):"), 0, 2)
        self.freq_step_input = QLineEdit("0")
        rf_layout.addWidget(self.freq_step_input, 0, 3)

        rf_layout.addWidget(QLabel("End (MHz):"), 1, 0)
        self.freq_end_input = QLineEdit("0")
        rf_layout.addWidget(self.freq_end_input, 1, 1)

        # RF Power Sweep Inputs
        rf_layout.addWidget(QLabel("RF Power Start (dBm):"), 2, 0)
        rf_layout.addWidget(self.rf_control_screen.power_start, 2, 1)

        rf_layout.addWidget(QLabel("Step (dB):"), 2, 2)
        rf_layout.addWidget(self.rf_control_screen.power_step, 2, 3)

        rf_layout.addWidget(QLabel("End (dBm):"), 3, 0)
        rf_layout.addWidget(self.rf_control_screen.power_end, 3, 1)

        rf_layout.addWidget(QLabel("Duration (s):"), 3, 2)
        rf_layout.addWidget(self.rf_control_screen.power_dur, 3, 3)
        ...
        for widget in self.config_widget.findChildren(QComboBox):
            ...
            widget.setView(QListView())  # Force use of styleable dropdown
        ...
        for widget in self.config_widget.findChildren(QPushButton):
            if widget != self.emergency_stop_button:  # Don't override red styling
    ...
    def run_next_frequency(self):
        if self.freq_index >= len(self.freq_list):
            self.log("All frequency sweeps completed.")
            self.handle_reset_ui()
            return

        if self.stop_event.is_set():
            self.log("Sweep was stopped. No further frequency steps will run.")
            self.handle_reset_ui()
            return

        freq = self.freq_list[self.freq_index]
        self.freq_index += 1

        try:
            self.exg_instr.write(f"FREQ {freq} Hz")
            self.log(f"Set EXG frequency to {freq / 1e6} MHz")
            if self.nrx_instr:
                self.nrx_instr.write(f"SENS:FREQ {freq} Hz")
        except Exception as e:
            self.log(f"[ERROR] Failed to set frequency: {e}")
            QTimer.singleShot(100, self.run_next_frequency)
            return

        # Keep thread references to avoid GC
        thread = QThread()
        worker = RFSweepWorker(
            self, self.rf_powers,
            self.vg_chan, self.vd_chan,
            self.vg_values, self.vd_values,
            self.vg_dur, self.vd_dur
        )
        worker.moveToThread(thread)

        self.thread_pool.append((thread, worker))  # store references

        def on_worker_done():
            thread.quit()
            thread.wait()
            self.thread_pool.remove((thread, worker))  # cleanup
            QTimer.singleShot(100, self.run_next_frequency)
    ...
    def run_sweep_threaded(self):
        try:
            ...
            try:
                f_start = float(self.freq_start_input.text()) * 1e6
                f_step = float(self.freq_step_input.text()) * 1e6
                f_end = float(self.freq_end_input.text()) * 1e6
            except ValueError:
                QMessageBox.warning(self, "Input Error", "Invalid RF frequency sweep values.")
                return

            frequencies = self.frange(f_start, f_end, f_step)
            ...
            for freq in frequencies:
                self.freq_list = self.frange(f_start, f_end, f_step)
                self.rf_powers = self.frange(rf_start, rf_end, rf_step)
                self.vg_chan = vg_chan
                self.vd_chan = vd_chan
                self.vg_values = vg_values
                self.vd_values = vd_values
