...
class RFSweepWorker(QObject):
    ...
    def run(self):
        ...
        # self.app.rf_sweep_finished.emit()
...
class NGP800IVSweepApp(QWidget):
    ...
    def __init__(self):
        ...
        # self.rf_sweep_finished.connect(self.plot_pae_vs_powerin)
    ...
    def log(self, message):
        ...
        if message.startswith("[RECORD]"):
            ...
            power_out = parts[6].split("=")[1] if len(parts) > 6 else "N/A"  # ← column 6
            power_in = parts[5].split("=")[1] if len(parts) > 5 else "N/A"   # ← column 5
            try:
                pout_dbm = float(power_out)
                pin_dbm = float(power_in)
                
                pout_w = 10 ** (pout_dbm / 10) * 0.001
                pin_w = 10 ** (pin_dbm / 10) * 0.001

                denominator = vd * curr
                pae = (pin_w - pout_w) * 100 / denominator if denominator > 0 else -1
                pae_str = f"{pae:.4f}"

                gain = pin_dbm - pout_dbm
                gain_str = f"{gain:.3f}"
            except:
                pae_str = "N/A"
                gain_str = "N/A"
            ...
            self.records_table.setItem(row, 7, QTableWidgetItem(gain_str))
            self.records_table.setItem(row, 8, QTableWidgetItem(pae_str))
    ...
    def setup_records_ui(self):
        ...
        self.records_table.setHorizontalHeaderLabels([
            ...
            "RF Freq (Hz)", "PowerIN (dBm)", "PowerOUT (dBm)", "GAIN (dB)", "PAE (%)"
        ])
    ...
    def run_sweep_threaded(self):
        ...
        self.combined_plot_widget.setLabel("bottom", "PowerIN (dBm)")
        ...
        self.rf_sweep_thread.finished.connect(self.plot_pae_vs_powerin)
    ...
    def plot_pae_vs_powerin(self):
        ...
        for row in range(self.records_table.rowCount()):
            try:
                pin = float(self.records_table.item(row, 5).text())
                pout = float(self.records_table.item(row, 6).text())
                gain = float(self.records_table.item(row, 7).text())
                pae = float(self.records_table.item(row, 8).text())
                pin_vals.append(pin)
        ...
        self.combined_plot_widget.plot(pin_vals, pout_vals, pen=pg.mkPen("b", width=2), name="PowerOUT (dBm)", symbol="o")
        self.combined_plot_widget.plot(pin_vals, pae_vals, pen=pg.mkPen("g", width=2), name="PAE (%)", symbol="x")
        self.combined_plot_widget.plot(pin_vals, gain_vals, pen=pg.mkPen("r", width=2), name="GAIN (dB)", symbol="t")
