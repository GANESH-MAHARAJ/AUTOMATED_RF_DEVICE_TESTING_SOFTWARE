...
class LimitsScreen(QWidget):
    ...
    def set_limits_with_callback(self, callback):
        try:
            freq = float(self.freq_input.text())
            freq_unit = self.freq_unit.currentText()
            power = float(self.power_input.text())
            ...
            callback(freq_limit_hz, (power, power_unit))
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Invalid input: {str(e)}")
...
class ControlScreen(QWidget):
    ...
    def init_ui(self):
        ...
        power_layout = QHBoxLayout()
        self.power_start = QLineEdit()
        self.power_step = QLineEdit()
        self.power_end = QLineEdit()
        self.power_dur = QLineEdit()
        self.power_start.setPlaceholderText("Start (e.g., -10)")
        self.power_step.setPlaceholderText("Step (e.g., 1)")
        self.power_end.setPlaceholderText("End (e.g., 0)")
        self.power_dur.setPlaceholderText("Duration (s)")

        self.power_unit = QComboBox()
        self.power_unit.addItems(["dBm", "dB", "W"])
        power_layout.addWidget(QLabel("Power:"))
        power_layout.addWidget(self.power_start)
        power_layout.addWidget(self.power_step)
        power_layout.addWidget(self.power_end)
        power_layout.addWidget(self.power_dur)
    ...
    def get_rf_power_sweep_values(self):
        try:
            start = float(self.power_start.text())
            step = float(self.power_step.text())
            end = float(self.power_end.text())
            dur = float(self.power_dur.text())
            return start, step, end, dur
        except:
            return None, None, None, None

    def set_rf_power(self, power_val):
        try:
            self.instr.write(f"POW {power_val} dBm")
            self.instr.write("OUTP ON")
        except Exception as e:
            print(f"RF set error: {e}")
    ...
    #         if self.freq_limit is not None and freq_hz > self.freq_limit:
    #             QMessageBox.critical(self, "Error", f"Frequency exceeds limit ({self.freq_limit} Hz)")
    #             return


    #         if self.power_limit and power_unit != self.power_limit[1]:
    #             QMessageBox.critical(self, "Error", f"Power unit must be {self.power_limit[1]}")
    #             return
    #         if self.power_limit and power_val > self.power_limit[0]:
