    ...
    def run(self):
        ...
        try:
            ...
            pinch_off_vgs = sorted(set(
                round(vg, 3)
                for (vg, vd, i) in self.data_points
                if round(vd, 6) == round(vd_max_val, 6) and 0 <= i <= 0.05
            ))
            print(pinch_off_vgs)
            ...
            # Calculate gm at 5 evenly spaced Vd values
            from collections import defaultdict
            vd_groups = defaultdict(list)
            for vg, vd, i in self.data_points:
                vd_groups[round(vd, 3)].append((vg, i))

            sorted_vds = sorted(vd_groups.keys())
            selected_vds = sorted_vds[::max(1, len(sorted_vds) // 5)]
            gm_values = []

            for vd in selected_vds:
                pairs = sorted(vd_groups[vd], key=lambda x: x[0])
                for j in range(1, len(pairs)):
                    ...
                    if vg2 != vg1:
                        ...
                        gm_values.append(gm)
            gm_avg = sum(gm_values) / len(gm_values) if gm_values else None

            self.parameters_ready.emit(idss, pinch_off, gm_avg)
        except Exception as e:
